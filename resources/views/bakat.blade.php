...
<html lang="en">
  ...
  <body class="bg-light">
    ...
    <div class="row bg-primary p-3 text-light">
        <h2> Jenis-Jenis Bakat </h2>
    </div>
    <div class="row bg-primary p-3 text-dark justify-content-center">
        @foreach ($data as $key=>$b)
        <div class="card me-md-4">
            <div class="card-body">
            <h5 class="card-title">{{ $b->bakat }}</h5> <hr>
            <img src="{{url('image/g'.$b->id.'.png')}}" class="mx-auto d-block" alt="bakat" width="130"><hr>
                <p class="card-text">Beberapa contoh profesi bakat {{ $b->bakat }} seperti : <br>
                     <b>{{$b->profesi}}, dan lainnya.</b></p>
                </div>
            </div>
        @endforeach
    </div>
